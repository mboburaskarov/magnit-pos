      ...
      if (false) {
        ...
        sendToEPOS({
          ...
          params: {
            ...
            items: mockData.flat(),
            receivedCash: paymentsList.filter((item) => item.amount && item.type === 'cash').reduce((sum, item) => sum + (item.amount || 0), 0) * 100, // Сумма полученной наличности. Значение указывается в тийинах (100 сум = 10000 тийин)
            receivedCard: paymentsList.filter((item) => item.amount && item.type !== 'cash').reduce((sum, item) => sum + (item.amount || 0), 0) * 100, // Сумма полученной безналичности. Значение указывается в тийинах (100 сум = 10000 тийин)
          },
          ...(SALE_TYPE === 'RETURN' && {
            refundInfo: (() => {
