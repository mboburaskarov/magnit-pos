    ...
    >
      <Box justifyContent='space-between' width='100%' display='inline-flex'>
        <Box pb={2.5} borderBottom='1px solid' borderColor='gray.200' width='100%' mt={1} justifyContent='space-between' display='inline-flex'>
          <Typography fontSize={24}>Топ Аптекаы</Typography>
          <ClientsIcon />
        </Box>
      </Box>

      {data?.length ? (
        <Box width='100%' display='flex' alignItems='center' flexDirection='column' mt={2}>
          {formattedData?.map((el, ind) => (
            <Box
              ...
              alignItems='center'
              width='100%'
              backgroundColor={ind % 2 === 0 ? 'orange.100' : 'background.default'}
              display='inline-flex'
              mt={0}
            >
