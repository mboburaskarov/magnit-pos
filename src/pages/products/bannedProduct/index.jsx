...
import CreateBannedProduct from './createBonusProduct'
...
export default function BannedProductPage() {
  ...
  const { columns, loading } = useSelector((state) => state.bannedProductTableColumns)
  ...
  const { mutate: deleteBannedProduct } = useMutation(requests.deleteBannedProduct, {
    onSuccess: () => {
      refetch().then(() => {
        ...
        navigate(`/products/banned-product${requestParams}`)
      })
  ...
  const bannedProductListFilter = useMemo(() => {
    return {
  ...
  const {
    ...
    isFetching: isFetchingbannedProductList,
    refetch,
  } = useQuery(['bannedProductList', bannedProductListFilter], () => requests.getProductBannedList(bannedProductListFilter))

  useEffect(() => {
    refetch()
  }, [bannedProductListFilter])

  useEffect(() => {
    const count = bannedProductList?.data?.data?._meta?.total_count

    const offsetsCount = Math.ceil(count / Number(values?.limit))
    setOffsetCount(offsetsCount || 0)
  }, [bannedProductList?.data, values?.limit])
        ...
        <Container>
          <FormProvider {...methods}>
            <Box display='flex' flexDirection='column' position='relative' px={'20px'} pb={'20px'}>
              ...
              <CreateBannedProduct refetch={refetch} open={openCreateBonusModal} setOpen={setopenCreateBonusModal} />
              <Box>
                <AgGridTable
                  id='auto-order-main-table'
                  tableSettings
                  columns={tableColumns}
                  data={bannedProductList?.data?.data?.data || []}
                  totalCount={bannedProductList?.data?.data?._meta?.total_count || 0}
                  isDataLoading={isFetchingbannedProductList || bannedProductListLoading}
                  ...
                  isRefreshing={loading || isFetchingbannedProductList || bannedProductListLoading}
                />
              </Box>
            </Box>
          ...
          {openConfirmDialog && (
            <ConfirmDialog
              ...
              title={'Удалить запрещенный продукт?'}
              desc={'Вы хотите Удалить запрещенный продукт?'}
              actions={
                <>
                  <Button
                  ...
                  <LoadingButton variant='contained' type='button' onClick={() => deleteBannedProduct(openConfirmDialog.id)}>
                    Да, удалить
                  </LoadingButton>
                </>
              }
