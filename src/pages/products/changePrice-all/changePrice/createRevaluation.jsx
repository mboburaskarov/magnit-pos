...
export default function CreateRevaluation({ open, refetch, setOpen }) {
  const methods = useForm()
  const [revaluationType, setRevaluationType] = useState('full')
  const [storeId, setStoreId] = useState(null)
  const [forceRerender, setForceRerender] = useState(0)

  const { reset, watch, control, setValue, clearErrors } = methods

  const { mutate: createRevaluation, isLoading: isCreateRevaluation } = useMutation(requests.createRevaluation, {
    onSuccess: () => {
      setOpen(false)
        ...
        <FormProvider {...methods}>
          <Box rowGap={3} flexWrap='wrap' display='flex' component='form' onSubmit={methods.handleSubmit(onSubmit, onError)}>
            ...
            <Box columnGap={2} display='flex' width='100%' mt={'4px'}>
              <LoadingButton loading={isCreateRevaluation} fullWidth variant='contained' type='submit' disabled={isCreateRevaluation}>
                {isCreateRevaluation ? t('Создание...') : t('filter_dialog.save.label')}
              </LoadingButton>
            </Box>
          </Box>
        </FormProvider>
