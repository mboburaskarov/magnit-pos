...
export default function OrderNote({ data }) {
  return (
    <Box key={data?._id} borderRadius={4} px={4} py={3} border={'2px dashed #eee'} display='inline-flex' width='100%' justifyContent='space-between'>
      <Box>
        ...
        <Box mt={1}>
          <Box alignItems='center' display='inline-flex'>
            ...
            <Typography ml={0.7} color='green.600' fontSize={18} lineHeight='24px'>
              {dayjs(data?.createdAt).isValid() ? dayjs(data?.createdAt).format('DD.MM.YYYY HH:mm') : '-'}
            </Typography>
          </Box>
          {data?.userInfo && (
