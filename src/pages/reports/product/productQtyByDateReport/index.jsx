...
import { changeColumnSequence, resetTableHeader, updateTableHeader } from '../../../../redux-toolkit/tableSlices/productQuantityByDateReportTableColumns'
import FilterMenu from './FilterMenu'
import tableHeaderSelector from './tableHeaderSelector'
const SELECTION_ID = 'checkboxSelectionField'

export default function ProductQtyByDateReportPage() {
  ...
  const { columns, loading } = useSelector((state) => state.productQuantityByDateReportTableColumns)
  ...
  const {
    data: productQtyByDateReportList,
    isLoading: productQtyByDateReportListLoading,
    isFetching: isFetchingproductQtyByDateReportList,
    refetch,
  } = useQuery(['productQtyByDateReportList', productQtyByDateReportListFilter], () =>
    requests.getProductQuantityByDateReport(productQtyByDateReportListFilter)
  )

  const { data: productReportDashboard, refetch: fetchProductReportDashboard } = useQuery(
    ['productReportDashboard', values?.search, productQtyByDateReportListFilter],
    () => requests.getProductReportStat(productQtyByDateReportListFilter)
  )
  ...
  useEffect(() => {
    refetch()
  }, [productQtyByDateReportListFilter])

  useEffect(() => {
    const count = productQtyByDateReportList?.data?.data?._meta?.total_count
    setselectClients([])
      ...
      <Box display='flex' mx={'auto'} flexDirection='column' position='relative' pt={'0px'} px={'50px'} pb={'20px'}>
        ...
        <Box>
          <AgGridTable
            ...
            isDataLoading={isFetchingproductQtyByDateReportList || productQtyByDateReportListLoading}
            ...
            isRefreshing={loading || isFetchingproductQtyByDateReportList || productQtyByDateReportListLoading}
          />
        </Box>
      </Box>
