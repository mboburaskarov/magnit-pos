...
export default function CompanyDrawer({ refetchCompanyList, openDrawer, closeDrawer }) {
  ...
  const { mutate: createComapny, isLoading: isCreateCustomer } = useMutation(requests.createComapny, {
    onSuccess: ({ data }) => {
      ...
      methods.reset()
      refetchCompanyList()
      success('Компания создан!')
    },
    onError: (err) => {
      error('Ошибка при Компания создан!')
  ...
  const { mutate: handleUpdateCompany, isLoading: isUpdateCompany } = useMutation(requests.updateCompany, {
    onSuccess: ({ data }) => {
      ...
      success('Компания успешно редактирование!')
    },
    onError: (err) => {
      error('Ошибка при редактирование Компания!')
      console.error('err', err)
    },
  })
      ...
      <Box height={'100%'}>
        ...
        <FormProvider {...methods}>
          <Box
            ...
            overflow={'auto'}
          >
            <form id='create-company' onSubmit={methods.handleSubmit(onSubmit, onError)}>
              <Box
                sx={{
                  padding: '0 24px',
                }}
