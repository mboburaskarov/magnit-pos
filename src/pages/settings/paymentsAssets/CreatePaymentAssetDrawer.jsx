...
import CreatePaymentAssetBody from './CreatePaymentAssetBody'

export default function CreatePaymentAsset({ isOpen, onClose, categoriesRefetch }) {
  const methods = useForm()

  const { mutate: createPaymentAsset, isLoading: createPaymentAssetLoading } = useMutation(requests.createPaymentAsset, {
    onSuccess: () => {
      categoriesRefetch()
      success('Ключ успешно создано!')
      onClose()
    },
    onError: (err) => {
      error('Ошибка при создании Ключ!')
    ...
    if (get(isOpen, 'mode') == 'edit') {
      const requestBody = {
        ...
        ...(get(data, 'type_action') != 'click' && {
          ...
          secret_key: get(data, 'merchant_key'),
        }),
  ...
  return (
    ...
    >
      <FormProvider {...methods}>
        <CreatePaymentAssetBody isOpen={isOpen} />
      </FormProvider>
    </CardDrawer>
  )
}
