  ...
  return (
    <Box
      sx={{
        ...
        background: 'linear-gradient(147.13deg, #868FAA 3.88%, #525A77 73.63%)',
      }}
    >
      <Box sx={{ display: 'flex', alignItems: 'start', flexDirection: 'column', justifyContent: 'space-between' }}>
        <Typography sx={{ fontSize: '14px', lineHeight: '20px', fontWeight: '500', color: 'bunker.100' }}>Карта лояльности</Typography>
        <Typography sx={{ fontSize: '28px', lineHeight: '40px', fontWeight: '700', color: 'white' }}>
          {thousandDivider(get(data, 'balance', '0000'), 'сум')}
        </Typography>
      </Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Typography sx={{ fontSize: '14px', lineHeight: '20px', fontWeight: '500', color: 'bunker.100' }}>
          {get(data, 'loyalty_card_barcode').replace(/.(?=.{4})/g, '*')}{' '}
        </Typography>
        <Box width={'115px'}>
          <BrandCardIcon />
        </Box>
