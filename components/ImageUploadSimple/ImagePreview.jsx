    ...
    <Box position='relative'>
      ...
      {uploadedImages?.length && withoutTextBox ? (
        <Box
          sx={{
            '&:hover': {
              ...
              cursor: 'pointer',
              '&:hover': { '#upload-img-preview': { transform: 'rotate(0deg) !important' }, img: { transform: 'rotate(0deg)' } },
            },
          }}
          position='relative'
          width={width || 128}
        ...
        >
          ...
          <Box display={'none'} className='img-actions'>
            <Box
              onClick={(e) => {
                e.stopPropagation()
                // setOpenGallery(true)
          ...
          {uploadedImages.slice(0, 3).map((el, ind) => (
            <Box
              ...
              key={ind}
            >
              <CustomImg src={getImageUrl(el.key)} alt={`image of ${el.key || 'product'}`} width={50} height={50} />
            </Box>
          ))}
        </Box>
      ) : (
